...
def apply_algorithm(q):
    ...
    # ★ NEW: Duplicate keyword in 2 options → narrow to those 2
    word_to_opts = {}
    ...
    if pair_words:
        # Pick the longest pair word (most specific)
        best_word = max(pair_words.keys(), key=len)
        idx_a, idx_b = pair_words[best_word]
        opt_a, correct_a = cleaned[idx_a]
        opt_b, correct_b = cleaned[idx_b]
        len_a = len(opt_a.split())
        len_b = len(opt_b.split())
        if len_a != len_b:
            # Pick the longer one (86% accuracy when length differs)
            if len_a > len_b:
                return (correct_a, 'gamble', f'DupKey({best_word}): longer')
            else:
                return (correct_b, 'gamble', f'DupKey({best_word}): longer')
        else:
            # Same length - pick first
            return (correct_a, 'gamble', f'DupKey({best_word}): same-len')
...
print("=" * 60)
print("📊 نتائج بعد إضافة نمط الكلمة المكررة")
print("=" * 60)
